...
export function UpgradesBoard({
  ...
  const subtitle = `${targetLabel}顯示 ${items.length}/${LIST_MAX}${
    data.candidates.length > LIST_MAX ? ` · 候選共 ${data.candidates.length}` : ""
  }`;
      ...
      <div style={styles.content}>
        ...
        <div style={styles.header}>
          <div style={styles.titleBlock}>
            ...
            <div style={styles.title}>加分推薦</div>
            <div style={styles.subtitle}>{subtitle}</div>
          </div>
        </div>

        {items.length === 0 ? (
          <div style={styles.empty}>暫無加分候選</div>
        ) : (
